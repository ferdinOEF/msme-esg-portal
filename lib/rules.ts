...
export function buildRecommendations(c: CompanyContext): Recommendations {
  const out: Recommendations = { mandatory: [], optional: [], schemes: [] };

  const push = (k: keyof Recommendations, v: string) => {
    out[k].push(v);
  };
